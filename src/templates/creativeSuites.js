...
class CreativeSuitesSingleTemplate extends React.Component {
	settings = {
		centerMode: true,
		centerPadding: '300px',
		dots: true,
	...
	render() {
		const pageDetail = this.props.data.allContentfulCreativeSuites.edges[0].node;
		...
		return (
			<div className="inner-page">
				<DefaultLayout headerData={this.props.data.allContentfulLayout.edges[0].node.header} footerData={this.props.data.allContentfulLayout.edges[0].node.footer}>
					<Helmet title={this.props.data.allContentfulCreativeSuites.edges[0].node.metaTitle}>
						<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css" />
						<link rel="icon" href={favIcon} type="image/x-icon" />
						{this.props.data.allContentfulCreativeSuites.edges[0].node.metaDescription ? <meta name="description" content={this.props.data.allContentfulCreativeSuites.edges[0].node.metaDescription} /> : null}
						{this.props.data.allContentfulCreativeSuites.edges[0].node.metaKeywords ? <meta name="keywords" content={this.props.data.allContentfulCreativeSuites.edges[0].node.metaKeywords} /> : null}
					</Helmet>
					...
					<div className="studio-single-page">
						...
						{pageDetail.otherSuites ? <div className="other-studios" data-aos="fade-in" data-aos-duration="2000">
							{pageDetail.otherSuites.map((other, key) => (
...
export default CreativeSuitesSingleTemplate

export const pageQuery = graphql`
  query CreativeSuitesSingle($slug: String!) {
    site {
      siteMetadata {
        title
      }
	...
	allContentfulLayout {
	  ...
	  allContentfulCreativeSuites(filter: {url: {eq: $slug}}) {
		edges {
		  node {
			metaTitle
			metaKeywords
